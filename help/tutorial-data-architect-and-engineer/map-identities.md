...
1. `Luma Loyalty Id`ID 名前空間 **[!UICONTROL ドロップダウンから]** 名前空間を選択します
...
1. `Luma CRM Schema` で、`crmId` 名前空間を使用して、`Luma CRM Id` フィールドにプライマリ ID のラベルを付けます
1. `Luma Offline Purchase Events Schema` で、`loyaltyId` 名前空間を使用して、`Luma Loyalty Id` フィールドにプライマリ ID のラベルを付けます
1. `Luma Product Catalog Schema` で、`productSku` 名前空間を使用して、`Luma Product SKU` フィールドにプライマリ ID のラベルを付けます
...
プライマリ ID を選択すると、両方とも loyaltyId を識別子として使用するの `Luma Loyalty Schema`、`Luma Offline Purchase Events Schema` にどう接続できるかがはっきりします。 しかし、CRM はどのようにオフライン購入イベントに接続できますか？ オフライン購入をオンライン行動と結び付けるにはどうすればよいですか？ 製品カタログで購入した製品を分類するにはどうすればよいですか？ 追加の ID フィールドとスキーマ関係を使用します。
...
1. フィールド `crmId` 選択
1. 「**[!UICONTROL ID]**」チェックボックスをオンにします
1. `Luma CRM Id`ID 名前空間 **[!UICONTROL ドロップダウンから]** 名前空間を選択します
...
1. `Luma Product Catalog Schema` 参照スキーマ **[!UICONTROL として「]**」を選択します
