...
1. 「**Params**」タブで、`luma-crm.json`filePath **として** と入力します
...
1. 「**パラメーター**」タブで、`COMPLETE` アクション **として** と入力します
...
最後に、`Luma CRM Id` 名前空間でプロファイルの 1 つを検索して（例：`f660ab912ec121d1b1e928a0bb4bc61b`）、いずれかのプロファイルが作成されたことを確認します。
...
`5625458` 名前空間で値 `loyaltyId` のプロファイルを再度検索して、プロファイルに購入イベントがあるかどうかを確認します。 1 つの購入が表示されます。 **[!UICONTROL JSON を表示]** を選択して、購入の詳細を調べることができます。
