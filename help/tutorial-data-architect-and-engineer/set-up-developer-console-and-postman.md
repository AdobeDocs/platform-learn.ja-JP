...
Adobe Developer Consoleにアクセスすると、Adobeの API と SDK、ほぼリアルタイムのイベントのリッスン、ランタイムでの関数の実行、プラグインまたはApp Builder アプリケーションのビルドを行うことができます。 これを使用して、Experience Platform API にアクセスします。 詳しくは、[Adobe Developer Consoleのドキュメントを参照してください &#x200B;](https://www.adobe.io/apis/experienceplatform/console/docs.html)
...
1. 使用可能な API の一覧で、「**[!UICONTROL Experience Platform API]**」を選択し、「**[!UICONTROL 次へ]**」を選択します。
...
API 資格情報を使用してExperience Platformとやり取りするには、システム管理者に、前のレッスンで作成したロールに API 資格情報を割り当てる必要があります。  システム管理者でない場合は、次のアドレスを送信します。
...
1. リストが長い場合は、チュートリアル参加者から提供された `Credential in Luma Tutorial API Project` テクニカルアカウントメール [!UICONTROL &#x200B; を使用してフィルタリングする、] しい資格情報を見つけます
1. 認証情報の選択
1. 「**[!UICONTROL 保存]**」を選択します
...
1. Web ブラウザーに切り替え、Experience Platformのインターフェイスに移動して URL の一部（@記号の後 *を抽出し、会社のテナント ID を検索し* す。 例えば、テナント ID は `techmarketingdemos` ですが、現在の ID は異なります。
...
Adobeには、Experience Platform API を調べるのに役立つ豊富な [!DNL Postman] コレクションが用意されています。 これらのコレクションは、[Adobe Experience Platform Postman サンプル GitHub リポジトリ &#x200B;](https://github.com/adobe/experience-platform-postman-samples) にあります。 このリポジトリーは、このチュートリアル全体で何度も使用するため、および後で自社にExperience Platformを実装する際に、ブックマークする必要があります。

最初のコレクションは、Adobe Identity Management サービス（IMS） API で動作します。 これは、Postman内からアクセストークンを取得する便利な方法です。
...
1. アクセストークンは、**環境の** ACCESS_TOKEN[!DNL Postman] 環境変数として自動的に保存される必要があります。
...
[Experience Platform [!DNL Postman] collections in GitHub](https://github.com/adobe/experience-platform-postman-samples/tree/master/apis/experience-platform) を開きます。 このページには、様々な Platform API 用の多くのコレクションがあります。 ブックマークすることを強くお勧めします。
