...
すべてのデータが取り込まれるようになりました。

## 3.8.1.3 Profile Target Dimension

Orchestrated Campaigns を使用すると、Adobe Experience Platformのリレーショナルスキーマ機能を活用して、ターゲットコミュニケーションを設計し、エンティティレベルで配信できます。 Experience Platform では、スキーマを使用して、一貫性のある再利用可能な方法でデータの構造を記述します。データが Experience Platform に取り込まれると、XDM スキーマに応じて構造化されます。

オーケストレートキャンペーンのセグメント化は主にリレーショナルスキーマに基づいて機能しますが、実際のメッセージ配信は常にプロファイルレベルで行われます。

ターゲティングを設定する際は、次の 2 つの重要な側面を定義します。

- ターゲット可能なスキーマ：ターゲット設定の対象となるリレーショナルスキーマを指定します。 デフォルトでは、受信者という名前のスキーマが使用されますが、訪問者、顧客などの代替を設定できます。

- プロファイルのリンケージ：システムは、ターゲットスキーマがプロファイルスキーマにどのようにマッピングされるかを理解する必要があります。 これは、共有 ID フィールドを通じて実現されます。共有 ID フィールドは、ターゲットスキーマとプロファイルスキーマの両方に存在し、ID 名前空間として設定されています。

次に、プロファイルのターゲットディメンションを設定する必要があります。 **管理**/**設定** に移動し、「**Profile Target Dimension** の下の **管理** をクリックします。
...
**スキーマ** には、`--aepUserLdap--_citisignal_accounts` を選択します。 **ID 値** には、**account_id** を選択します。
...
**スキーマ** には、`--aepUserLdap--_citisignal_recipients` を選択します。 **ID 値** には、**account_id** を選択します。
...
**スキーマ** には、`--aepUserLdap--_citisignal_recipients` を選択します。 **ID 値** には、「**メール**」を選択します。
...
左側のメニューで、**チャネル** に移動します。 **チャネル設定を作成** をクリックします。

![AJO OC](./images/ajoocchannel1.png)

名前 `--aepUserLdap---citisignal-email-oc` を使用します。 チャネル **メール** を選択します

![AJO OC](./images/ajoocchannel2.png)

使用可能な **サブドメイン** および **IP プール** を選択します。

![AJO OC](./images/ajoocchannel3.png)

下にスクロールして、**ヘッダーパラメーター** を入力します。

- **送信者名**: `info@techinsiders.org`
- **送信元メールのプレフィックス**: `info`
- **返信先名**: `info@techinsiders.org`
- **メールへの返信**: `info@techinsiders.org`
- **エラーの E メール プレフィックス**: `error`

![AJO OC](./images/ajoocchannel4.png)

下にスクロールします。 **キャンペーンの調整** に移動します。 **有効** のチェックボックスをオンにします。 **ターゲティングディメンション** を選択します。

![AJO OC](./images/ajoocchannel5.png)

**Profile Target Dimension** で、「`--aepUserLdap--_citisignal_recipients - email`」を選択します。

**Target Dimension** を選択し、「**フォルダーを開く**」アイコンをクリックします。

![AJO OC](./images/ajoocchannel6.png)

「`--aepUserLdap--_citisignal_recipients`」をクリックします。

![AJO OC](./images/ajoocchannel7.png)

フィールド **メール** を選択し、「**選択**」をクリックします。
...
次の演習では、調整されたキャンペーンの一部としてそのデータを使用します。
