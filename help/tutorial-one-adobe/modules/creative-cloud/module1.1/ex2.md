...
## 1.1.2.4 手動によるファイルのアップロードと、スタイル参照としてのグラデーションファイルの使用

これで、任意のグラデーションファイルをコンテナにアップロードする必要があります。 任意のグラデーション ファイルを使用できます。または、コンピュータにダウンロードして [ このファイル ](./images/gradient.jpg) を使用できます。

![Azure ストレージ ](./images/gradient.jpg)

Azure ストレージエクスプローラーで、グラデーションファイルをコンテナにドロップします。
...
プレースホルダー URL を、Azure ストレージエクスプローラーからコピーしたグラデーションファイルの事前署名済み URL に置き換えます。 これで完了です。 「**送信**」をクリックします。
...
次に、`horses in a field` を含む別の画像が表示されますが、今回はスタイルはスタイル参照として指定したグラデーションファイルに類似しています。
...
ローカルマシンからファイルを選択する必要があります。 任意の新しい画像ファイルを使用するか、別のグラデーションファイルを使用できます [ こちら ](./images/gradient2-p.jpg)。
