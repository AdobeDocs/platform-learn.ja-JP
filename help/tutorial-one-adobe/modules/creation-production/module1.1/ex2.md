...
1. 名前に `--aepUserLdap--` を使用し、「作成 **を選択し** す。
...
1. ブラウザーで応答Firefly サービスの新しい画像を開きます。
...
1. Postmanに戻り、フォルダー **FF - Firefly Services Tech Insiders** を選択し、「**Firefly**」フォルダーで「**...**」を選択してから「**リクエストを追加**」を選択します。

   ![Azure ストレージ ](./images/az30.png){zoomable="yes"}

1. 空のリクエストの名前を **Azure ストレージアカウントにファイルをアップロード** に変更し、**リクエストタイプ** を **PUT** に変更して、URL セクションに SAS-token URL を貼り付け、「**本文**」を選択します。
