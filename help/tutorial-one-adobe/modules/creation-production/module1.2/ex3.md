...
# 1.2.3 Frame.io とWorkfront Fusion
...
## 1.2.3.3 Workfront Fusion と Frame.io
...
## 1.2.3.4 Frame.io V4 カスタムアクション API
...
[https://next.frame.io/](https://next.frame.io/){target="_blank"} に戻り、プロジェクト **で作成したフォルダー** CitiSignal Fiber Campaign`--aepUserLdap--` に移動します。 ページを更新します。
...
## 1.2.3.5 Frame.io へのカスタムフォーム応答の提供
...
Frame.io に戻り、アセット `--aepUserLdap-- - Frame IO Custom Action Fusion`citisignal-fiber.psd **のカスタムアクション** ージをもう一度クリックします。
...
Frame.io に戻り、アセット `--aepUserLdap-- - Frame IO Custom Action Fusion V4`citisignal-fiber.psd **のカスタムアクション** ージをもう一度クリックします。
...
## 1.2.3.7 別のWorkfrontを呼び出すシナリオ

前の演習では、シナリオ `--aepUserLdap-- - Firefly + Photoshop` を設定しました。 次に、そのシナリオに小さな変更を加える必要があります。

別のタブでシナリオ `--aepUserLdap-- - Firefly + Photoshop` を開き、最初の **Adobe Photoshop - PSDの編集内容を適用** モジュールをクリックします。 これで、入力ファイルがMicrosoft Azureの動的な場所を使用するように設定されていることがわかります。 このユースケースでは、入力ファイルはMicrosoft Azureに保存されなくなり、代わりに Frame.io ストレージを使用するので、これらの設定を変更する必要があります。
...
## 1.2.3.9 エンドツーエンドのユースケースのテスト

シナリオ **ージで「** 1 回実行 `--aepUserLdap-- - Frame IO Custom Action`」をクリックします。
...
Workfront Fusion に戻ります。 シナリオ **ージで「** 1 回実行 `--aepUserLdap-- - Frame IO Custom Action V4`」をクリックします。
...
[1.2.4 Frame.io から Fusion に移動して、AEM Assetsに移動します ](./ex4.md){target="_blank"}
