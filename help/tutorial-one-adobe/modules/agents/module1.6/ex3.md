...
次に、ChatGPT を使用して、コンテンツフラグメントをさらなるバリエーションに翻訳します。 次のプロンプトを入力し、「**送信**」をクリックします。
...
選択した各言語に対して、この手順を繰り返します。 完了すると、次のようになります。

![Agent Orchestrator](./images/chatgpt26.png)

AEM UI に戻り、画面を更新します。 これで、コンテンツフラグメントに新しいバリエーションが表示されます。
