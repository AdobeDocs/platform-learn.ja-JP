---
title: Collaboration measurement - セットアップとレポート作成
description: Real-Time CDP measurementをインサイトに富んだキャンペーンレポート用に設定する方法を説明します。
feature: Collaboration
role: Admin, User
level: Intermediate
doc-type: Feature Video
duration: 604
last-substantial-update: 2026-05-05T00:00:00Z
jira: KT-21127
source-git-commit: e6f543c7dcc90116fef0ff1d1a1830334b80303f
workflow-type: tm+mt
source-wordcount: '71'
ht-degree: 0%

---


# Collaboration measurement - セットアップとレポート作成

Real-Time CDP Collaborationで測定を設定する方法について説明します。 このガイドでは、パブリッシャーキャンペーンデータにリンクされたレポートを作成できるようにするためのオンボーディングイベントデータについて説明します。 正確な測定は、施策のパフォーマンスを分析し、今後の戦略を最適化するのに役立ちます。 詳細な手順とインサイトについては、動画をご覧ください。

>[!VIDEO](https://video.tv.adobe.com/v/3486343/?learn=on&enablevpops)
