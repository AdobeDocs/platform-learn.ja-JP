...
プロジェクトを開く `--demoProfileLdap-- - Omnichannel Analysis`.
...
次に、表で最初の 5 つを選択します。 **ユーザーエージェント**  行を **Google Pixel XL 32GB ブラックスマートフォン** > **シティ信号**. 3 行を選択する際、 **CTRL** ボタン（Windows の場合）または **コマンド** ボタン (Mac)
