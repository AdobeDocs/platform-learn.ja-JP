---
title: データを一元化し、マーケティング上の意思決定を向上させるオーディエンスポータル
description: Adobe Real-Time CDPのオーディエンスポータルが、高度なオーディエンス管理とリアルタイムのアクティベーションを実現して、どのように顧客データを統合するかをご確認ください。
doc-type: Video Clips
duration: 83
index: false
hide: true
hidefromtoc: true
recommendations: noCatalog, noDisplay
exl-id: ffa91fcc-4cb0-4597-a7dc-9dc8c0850a06
source-git-commit: fe8d5b8315048d91baf31dc7d8168bddaec56110
workflow-type: tm+mt
source-wordcount: '54'
ht-degree: 0%

---

# オーディエンスポータル：データを一元化し、マーケティング上の意思決定を向上

Adobe Real-Time CDPのオーディエンスポータルが、高度なオーディエンス管理とリアルタイムのアクティベーションを実現して、どのように顧客データを統合するかをご確認ください。

<!-- 72_S508_3442517_82_audience-portal-centralizing-data-for-better-marketing-decisions -->
>[!VIDEO](https://video.tv.adobe.com/v/3458185/?learn=on&enablevpops=true)
