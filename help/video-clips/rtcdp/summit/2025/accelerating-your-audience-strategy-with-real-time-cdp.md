---
title: Real-Time CDPでオーディエンス戦略を加速
description: Real-Time CDPを活用して、データを統合し、エクスペリエンスをパーソナライズして、エンゲージメントを促進することで、オーディエンス戦略を加速する方法をご確認ください。
doc-type: Video Clips
duration: 46
index: false
hide: true
hidefromtoc: true
recommendations: noCatalog, noDisplay
exl-id: b4155fc1-aa0f-4e7b-bbfb-f6833c04c300
source-git-commit: fe8d5b8315048d91baf31dc7d8168bddaec56110
workflow-type: tm+mt
source-wordcount: '52'
ht-degree: 0%

---

# Real-Time CDPでオーディエンス戦略を加速

Real-Time CDPを活用して、データを統合し、エクスペリエンスをパーソナライズして、エンゲージメントを促進することで、オーディエンス戦略を加速する方法をご確認ください。

<!-- 62_S508_3442517_45_accelerating-your-audience-strategy-with-realtime-cdp -->
>[!VIDEO](https://video.tv.adobe.com/v/3459606/?captions=jpn&learn=on&enablevpops=true)
