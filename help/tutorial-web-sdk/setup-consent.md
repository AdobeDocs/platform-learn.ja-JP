...
1. 次の名前のサービスを追加 `aep web sdk` をオンにし、 **サービスのデフォルトの状態**. オンにすると、デフォルトの同意値は次のようになります。 `true`、それ以外の場合は `false`. この設定は、Web アプリケーションに対して（訪問者の同意前の）デフォルトの同意状態を決定する場合に便利です。 例：
