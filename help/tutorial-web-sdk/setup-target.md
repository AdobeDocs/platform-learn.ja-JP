...
>[!NOTE]
>
>このチュートリアルでは、[Luma web サイト ](https://newluma.enablementadobe.com){target=_blank} を使用します。このサイトでは、タグとちらつきの軽減が非同期で実装されています。 この節では、ちらつきの軽減が Platform Web SDKでどのように機能するかを説明します。
...
1. **[!UICONTROL 事前非表示スタイルを編集]** ボタンを選択します
   ...
   ![ 同期実装のための Target 事前非表示スニペット ](assets/target-flicker-sync.png)
...
Luma サイトを使用するこのチュートリアルでは、`lumaCrmId`ID[ に関す ](configure-identities.md) レッスンで設定した ID 記号を使用します。
...
1. `all pages - adobeDataLayer push - send event - 50` ルールを開きます
...
1. アクティビティ URL の Luma ホームページを使用してエクスペリエンスのターゲット設定（XT）アクティビティを作成し、Target ワークスペースを使用する（存在する場合）。
...
1. アクティビティ名を更新し、「**[!UICONTROL 次へ]**」を選択します。
...
1. [Luma デモサイト ](https://newluma.enablementadobe.com) に移動し、デバッガーを使用して [ サイトのタグプロパティを独自の開発プロパティに切り替える ](validate-with-debugger.md#use-the-experience-platform-debugger-to-map-to-your-tags-property)
1. ページをリロードします。
...
1. ルールを Luma ホームページに制限する条件を追加します（クエリ文字列のないパスは `/` または `/index.html` に等しい）。
...
1. 次のコードを「コンテンツ」ボックスに貼り付けて、ヒーローのcall-to-actionに変更を加えます。
...
>[!NOTE]
>
>「mbox をクリック」コンバージョン目標は自動的には機能しません。 Platform Web SDKはカスタム範囲を自動的にレンダリングしないので、コンテンツを適用するために選択した場所へのクリックを追跡しません。 「クリック」 `eventType` ールと `_experience` アクションを使用した該当する `sendEvent` の詳細を使用して、各範囲に対して独自のクリックの追跡を作成できます。
...
XDM オブジェクトからマッピングされていない、Target に役立つパラメーターがあります。 これらの特別な Target パラメーターには、以下が含まれます。
...
これらのパラメーターは、`data` オブジェクトではなく `xdm` オブジェクトで送信する必要があります。 また、`data` オブジェクトに page （または mbox）パラメーターを含めることもできます。

データオブジェクトにデータを入力するには、次のデータ要素を作成します。その際、[ データ要素の作成 ](create-data-elements.md) レッスンで作成したデータ要素を再利用します。

1. タグルール `product detail pages - adobeDataLayer push - set product details variables - 20` を開きます。
...
1. **[!UICONTROL target]** オブジェクトを選択します
1. `entity.id` を `Ecommerce Product Id` データ要素として設定します
1. `entity.name` を `Ecommerce Product Name` データ要素として設定します
1. `user.categoryId` を `Ecommerce Product Category` データ要素として設定します
1. 「**[!UICONTROL 変更を保持]**」を選択します
1. 「**[!UICONTROL 保存]**」を選択します
...
1. `all pages - adobeDataLayer push - send event - 50` ルールを開きます
1. `Adobe Experience Platform Web SDK - Send event` アクションを選択します
1. `Data Variable` データ要素がまだ追加されていない場合は、データフィールドに追加します
...
Luma サイトでは、データレイヤーは素早く読み込まれ、イベント送信の呼び出しが実行される前に完全に定義されます。 これにより、1 回の呼び出しを使用して、パーソナライズされたコンテンツの取得（Adobe Targetからなど）と、分析データの送信（Adobe Analyticsへなど）の両方を行うことができます。
...
2 回目の呼び出しは、パーソナライズされたコンテンツのすべてのビューのレポート作成と、`decisioning.propositionDisplay` 回の呼び出しの置き換えにも対応します。
...
1. `all pages - window loaded - send event - 50` という名前の新しいルールを作成します。
1. ルールにイベントを追加します。 **Core** 拡張機能と **[!UICONTROL Window Loaded]** イベントタイプの使用
...
1. これにより、「**[!UICONTROL レンダリングされた提案を含める]**」チェックボックスが選択されたままになるので、決定リクエストからのキュー内の表示通知が送信されます。
1. 「**[!UICONTROL 変更を保持]**」を選択します
1. ルール **[!UICONTROL 保存]**
1. ライブラリの更新と再構築
...
>[!TIP]
>
>で決定の提案を取得しているイベントで、後続のAdobe Analytics イベントがない場合、「**[!UICONTROL ガイド付きイベントを使用]** はオフのままになります。 すべてのオプションを手動で選択する必要がありますが、これによりオプションのロックが解除され、取得リクエストに加えて **[!UICONTROL レンダリングされた提案を含める]** ことができます。
...
1. [Luma デモサイト ](https://newluma.enablementadobe.com) に移動します
1. 製品詳細ページへの移動
