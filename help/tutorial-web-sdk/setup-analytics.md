...
1. タグルール `all pages - adobeDataLayer push - set global variables - 1` を開きます。
...
1. `eVar1` を `Page Name` データ要素として設定します
1. `prop1` を設定して、`eVar1` の値をコピーします
1. `data` オブジェクトが XDM にマッピングされた値をどのように上書きするかを示すには、「**[!UICONTROL 追加のプロパティ]**」セクションで、ページ名を静的な値として設定します `test`
...
1. タグルール `all pages - adobeDataLayer push - send event - 50` を開きます。
...
訪問者が特定のページを閲覧する際に使用するAdobe Analytics レポートスイートを変更できます。 これには、データストリームとルールの両方で設定が必要です。
...
1. 新しいルールを作成し、`homepage - adobeDataLayer push - AA report suite override - 51` という名前を付けます
...
1. 選択して **[!UICONTROL 詳細オプション]** を開き、`51` を入力します。 これにより、`all pages - adobeDataLayer push - send event - 50` 変数を更新 **[!UICONTROL アクションタイプを使用してベースライン XDM を設定する]** の後にルールが実行されます。
1. 「**[!UICONTROL 変更を保持]**」を選択します
...
1. **[!UICONTROL path equals]** で、`/` `OR` `/index.html` 設定します。 Luma デモサイトの場合、ルールがホームページにのみトリガーされるようにします
...
1. **[!UICONTROL XDM データ]** として、`XDM Variable` データ要素の作成 [&#x200B; レッスンで作成した &#x200B;](create-data-elements.md) データ要素を選択します
...
1. [Luma デモサイト &#x200B;](https://newluma.enablementadobe.com){target="_blank"} に移動
...
1. Luma サイトで：
   ...
   >[!NOTE]
   >
   >ログインしたので、`f660ab912ec121d1b1e928a0bb4bc61b` でも取得されたユーザーの認証済み ID **`test@test.com`** を検証 `[!UICONTROL c.a.x.identitymap.lumacrmid.[0].id]` ます

### レポートスイート上書き検証

上記で、[Luma ホームページ &#x200B;](https://newluma.enablementadobe.com) のデータストリーム上書きを設定しました。  この設定を検証するには
...
[Livingston All-Purpose Tight 製品ページ &#x200B;](https://newluma.enablementadobe.com/product.html?id=LLMP09) などの製品ページに移動します。  コンテンツページビューが Analytics によってキャプチャされていることを検証します。
...
1. また、製品ページを開いているので、`[!UICONTROL c.a.x.eventType]` は `commerce.productViews` に設定されていることにも注意してください。
   ...
   >[!TIP]
   >
   > `product detail pages - adobeDataLayer push - set product details variables - 20` ルールは、シーケンスの後半でトリガーに設定され `eventType` ので、`all pages - adobeDataLayer push - set global variables - 1` ルールで設定された値を上書きします
...
1. [Marco Lightweight Active Hoodie](https://newluma.enablementadobe.com/product.html?id=LLMH13) をカートに追加します。
1. [&#x200B; 買い物かごページ &#x200B;](https://newluma.enablementadobe.com/cart.html) に移動し、次のEdge Trace を確認します
...
>[!TIP]
>
> このレッスンを完了したら、`homepage - adobeDataLayer push - AA report suite override - 51` のルールを無効にすることをお勧めします。
