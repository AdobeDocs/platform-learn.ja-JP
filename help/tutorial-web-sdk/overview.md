...
このチュートリアルでは、Luma と呼ばれるサンプルの小売 web サイトでの Platform Web SDKの実装について説明します。 [Luma サイト &#x200B;](https://newluma.enablementadobe.com) には、現実的な実装を構築できる豊富なデータレイヤーと機能があります。 このチュートリアルでは、次の操作を行います。

* Luma web サイトの Platform Web SDK実装を使用して、独自のアカウントで独自のタグプロパティを作成します。
* データストリーム、スキーマ、ID 名前空間など、web SDKの実装で使用される主要なデータ収集機能を設定します。
...
>[!NOTE]
>
>[&#x200B; モバイルSDK](../tutorial-mobile-sdk/overview.md) についても、同様のチュートリアルを利用できます。
...
これらのレッスンでは、Adobe ユーザーアカウントと、レッスンを完了するために必要な権限があることを前提としています。 そうでない場合、アクセス権を取得するには、会社のExperience Cloud管理者に問い合わせる必要があります。
...
* オプションの **Journey Optimizer** レッスンでは、**[!UICONTROL キャンペーンを管理]**、**[!UICONTROL キャンペーンを公開]** および **[!UICONTROL キャンペーンレポートを表示]** するための権限項目が必要です。
  <!--
  ...
  * See the documentation for more information on [Journey Optimizer permission configuration](https://experienceleague.adobe.com/ja/docs/journey-optimizer/using/access-control/high-low-permissions#campaign-capability).
  -->
...
* 2026 年 2 月 27 日（PT）: イベント駆動型データレイヤーを備えた新しい Luma web サイト。
...
[Luma web サイト &#x200B;](https://newluma.enablementadobe.com){target="blank"} を別のブラウザータブに読み込んでブックマークすると、チュートリアル中に必要に応じて簡単に読み込むことができます。 ホストされている実稼動サイトを読み込める以外に、Luma への追加アクセスは必要ありません。

[![Luma web サイト &#x200B;](assets/overview-luma.png)](https://newluma.enablementadobe.com){target="blank"}

それでは、始めましょう。 次のトピック：[Web データ用の XDM スキーマの作成 &#x200B;](configure-schemas.md)
